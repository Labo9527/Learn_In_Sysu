...
http.createServer(function (request,response) {
    ...
    if (request.url == '/S3/index.html') {
      response.writeHead(200, {'Content-Type': 'text/html'});
      response.end(fs.readFileSync('index.html'));
    }
    else if (request.url == '/S3/style.css') {
      response.writeHead(200, {'Content-Type': 'text/css'});
      response.end(fs.readFileSync('style.css'));
    }
    else if (request.url == '/S3/S3.js') {
      response.writeHead(200, {'Content-Type': 'application/x-javascript'});
      response.end(fs.readFileSync('S3.js'));
    }
    else if (request.url == '/S3/assets/images/favicon.png') {
      response.writeHead(200, {'Content-Type': 'image/png'});
      response.end(fs.readFileSync('./assets/images/favicon.png'));
    }
    else if (request.url == '/S3/assets/images/atplus_white.png') {
      response.writeHead(200, {'Content-Type': 'image/png'});
      response.end(fs.readFileSync('./assets/images/atplus_white.png'));
    }
    else if (request.url == '/S3/assets/images/atplus_green.png') {
      response.writeHead(200, {'Content-Type': 'image/png'});
